...
import com.georsoft.common.utils.bean.BeanValidators;
import com.georsoft.common.utils.spring.SpringUtils;
import com.georsoft.system.domain.UsrPost;
import com.georsoft.system.domain.SysUserPost;
...
import com.georsoft.system.mapper.UsrUserMapper;
import com.georsoft.system.mapper.UsrUserPostMapper;
import com.georsoft.system.mapper.UsrUserRoleMapper;
...
{
    private static final Logger log = LoggerFactory.getLogger(UsrUserServiceImpl.class);
    ...
    {
        List<UsrRole> list = roleMapper.selectRolesByUserName(userName);
        if (CollectionUtils.isEmpty(list))
        {
            return StringUtils.EMPTY;
        }
        return list.stream().map(UsrRole::getRoleName).collect(Collectors.joining(","));
    }
    ...
    {
        List<UsrPost> list = postMapper.selectPostsByUserName(userName);
        if (CollectionUtils.isEmpty(list))
        {
            return StringUtils.EMPTY;
        }
        return list.stream().map(UsrPost::getPostName).collect(Collectors.joining(","));
    }
    ...
    public boolean checkUserNameUnique(UsrUsers user)
    {
        ...
        UsrUsers info = userMapper.checkUserNameUnique(user.getUserName());
    ...
    {
        Long userId = StringUtils.isNull(user.getId()) ? -1L : user.getId();
        UsrUsers info = userMapper.checkPhoneUnique(user.getMobileNo());
        if (StringUtils.isNotNull(info) && info.getId().longValue() != userId.longValue())
        {
            return UserConstants.NOT_UNIQUE;
        }
    ...
    {
        Long userId = StringUtils.isNull(user.getId()) ? -1L : user.getId();
        UsrUsers info = userMapper.checkEmailUnique(user.getEmail());
        if (StringUtils.isNotNull(info) && info.getId().longValue() != userId.longValue())
        {
            return UserConstants.NOT_UNIQUE;
        }
    ...
    public void checkUserDataScope(Long userId)
    {
        ...
        {
            UsrUsers user = new UsrUsers();
            user.setId(userId);
            List<UsrUsers> users = SpringUtils.getAopProxy(this).selectUserList(user);
     ...
     * @param user 用户对象
     */
    public void insertUserRole(UsrUsers user)
    {
        this.insertUserRole(user.getId(), user.getRoleCodes());
    }
     ...
     * @param user 用户对象
     */
    public void insertUserPost(UsrUsers user)
    {
    ...
    public int deleteUserByIds(Long[] userIds)
    {
        ...
        {
            checkUserAllowed(new UsrUsers(userId));
    ...
    public String importUser(List<UsrUsers> userList, Boolean isUpdateSupport, String operName)
    {
        ...
        for (UsrUsers user : userList)
        {
            ...
            {
                // 验证是否存在这个用户
                UsrUsers u = userMapper.selectUserByUserName(user.getUserName());
